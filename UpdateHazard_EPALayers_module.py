""" updates NPL, SEMS (CERCLIS), and Toxic Release Inventory """

from NaturalHazardUpdaterTool_Functions import *


def runEPALayers(workspace, chrome_driver_path, log_file_path, naturalhazards_gdb):
    ...
    hazard_nickname = "EPA_Hazards"

    # [original_name, NHD_hazard_name]
    tri_layer_name = ["TRI", "Toxics_Release_Inventory"]
    sems_layer_name = ["SEMS", "CERCLIS"]
    npl_layer_name = ["SEMS_NPL", "Superfund_Sites"]

    # these are the [[expected,required],...] fields
    sems_fields = [['PGM_SYS_ID', 'CERCLIS_EPA_ID'], ['PGM_SYS_ID', 'CERCLIS_EP'], ['PRIMARY_NAME', 'Site_Name'],
                   ['LOCATION_ADDRESS', 'Address'], ['ACTIVE_STATUS', 'NPL_Status']]

    npl_fields = [['PRIMARY_NAME', 'Name'], ['PGM_SYS_ID', 'EPA_ID'], ['LOCATION_ADDRESS', 'Address']]

    tri_fields = [['PRIMARY_NAME', 'FACILITY']]
    tri_dummy_field = 'CHEMICAL'

    tri_webaddress_field = {"name": "Report_URL",
                            "type": "TEXT",
                            "length": 150,
                            "base_url": r"https://enviro.epa.gov/facts/tri/ef-facilities/#/Facility/",
                            "id_field": "PGM_SYS_ID"}
